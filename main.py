...
async def lifespan(app: FastAPI):
    ...
    # Only start scheduler on one worker to prevent duplicate job execution
    # Option 1: Use ENABLE_SCHEDULER environment variable (set to "true" on one worker)
    # Option 2: Auto-detect first worker (worker-0 or ends with -0)
    enable_scheduler_env = os.getenv("ENABLE_SCHEDULER", "").lower() in ("true", "1", "yes")
    is_first_worker = WORKER_ID.endswith("-0") or WORKER_ID == "worker-0" or WORKER_ID.split("-")[-1] == "0"
    enable_scheduler = enable_scheduler_env or (not os.getenv("ENABLE_SCHEDULER") and is_first_worker)
    
    if enable_scheduler:
        logger.info(f"Starting scheduler on worker {WORKER_ID} (designated scheduler worker)...")
        ...
        logger.info(f"Skipping scheduler startup on worker {WORKER_ID} (not designated scheduler worker)")
