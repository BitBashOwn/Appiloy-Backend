...
def schedule_recurring_job(command: dict, device_ids: List[str]) -> None:
    ...
    random_minutes = random.randint(0, time_window_minutes - int(command.get("duration", 0)))
...
async def send_command(request: CommandRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if durationType in ['DurationWithExactStartTime', 'ExactStartTime']:
            ...
            schedule_single_job(target_time_utc, target_end_time_utc, device_ids, command, job_id, task_id)
            ...
            if abs(time_window - duration) <= 10:
                job_id = f"cmd_{uuid.uuid4()}"
                command['job_id'] = job_id
                schedule_single_job(start_time, end_time, device_ids, command, job_id, task_id)
            else:
                random_durations, start_times = generate_random_durations_and_start_times(duration, start_time, end_time)
                schedule_split_jobs(start_times, random_durations, device_ids, command, task_id)
